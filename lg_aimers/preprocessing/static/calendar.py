...
def add_holiday_info(df):
    ...
    day_diff = df["date"].diff().dt.days.fillna(1)
    block_start = (df["is_offday"].eq(1) & # offday가 시작되는 지점(True)만 누적합 → block id
                   (df["is_offday"].shift(fill_value=0).eq(0) | (day_diff != 1)))
    df["offblock_id"] = np.where(df["is_offday"].eq(1), block_start.cumsum(), np.nan)
    df["offblock_len"] = df.groupby("offblock_id")["is_offday"].transform("count")
    df["offblock_pos"] = df.groupby("offblock_id").cumcount()
    df = df.drop(columns="offblock_id")

    # 최근/다음 공휴일까지 일수
    # 비휴일(0)에 대해 최근/다음 휴일(1)까지 거리 계산
    df["is_holiday_int"] = df["is_holiday"] # For ffill/bfill
    df["days_since_prev_holiday"] = df.loc[df["is_holiday_int"].eq(1), "date"].reindex(df["date"].index, method="ffill")
    df["days_since_prev_holiday"] = (df["date"] - df["days_since_prev_holiday"]).dt.days.fillna(1000)
    df["days_to_next_holiday"] = df.loc[df["is_holiday_int"].eq(1), "date"].reindex(df["date"].index, method="bfill")
    df["days_to_next_holiday"] = (df["days_to_next_holiday"] - df["date"]).dt.days.fillna(1000)
    df = df.drop(columns="is_holiday_int")
    return df
